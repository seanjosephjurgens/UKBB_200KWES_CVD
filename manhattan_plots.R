...
multi_manhattan <- function(x, chr="CHR", bp="BP", p="P", snp="SNP", pch='pch',
                      ...
                      highlight=NULL, logp=TRUE, annotatePval = NULL, annotateTop = TRUE, textxy.cex = 3, ...) {
  ...
  d=data.frame(CHR=x[[chr]], BP=x[[bp]], P=x[[p]], pch=x[[pch]])
  ...
  #	 xlab=xlabel, ylab=expression(-log[10](italic(p))), las=1, pch=as.integer(x$pch), ...)


  # The new way to initialize the plot.
  ## See http://stackoverflow.com/q/23922130/654296
  ## First, define your default arguments
  def_args <- list(xaxt='n', bty='n', xaxs='i', yaxs='i', las=1, pch=as.integer(d$pch),
  ...
  bg=rep(col, max(d$CHR))
  # Add points to the plot
  if (nchr==1) {
    with(d, points(pos, logp, pch=as.integer(x$pch), col=col[1], bg=bg[1], ...))
  } else {
    # if multiple chromosomes, need to alternate colors and increase the color index (icol) each chr.
    icol=1
    for (i in unique(d$index)) {
      with(d[d$index==unique(d$index)[i], ], points(pos, logp, col=col[icol], bg=bg[icol], pch=as.integer(d[d$index==unique(d$index)[i], 'pch']), ...))
      icol=icol+1
    }
  }

  # Add suggestive and genomewide lines
  if (suggestiveline) abline(h=suggestiveline, col="blue", lty=ltynum)
  if (genomewideline) abline(h=genomewideline, col="red", lty=ltynum)

  # Highlight snps from a character vector
  if (!is.null(highlight)) {
    if (any(!(highlight %in% d$SNP))) warning("You're trying to highlight SNPs that don't exist in your results.")
    d.highlight=d[which(d$SNP %in% highlight), ]
    with(d.highlight, points(pos, logp, col="green3", pch=as.integer(d$pch), ...))
  }
  ...
  if (!is.null(annotatePval)) {
    ...
    if (annotateTop == FALSE) {
      with(subset(d, P <= annotatePval),
           textxy(pos, -log10(P), offset = 0.625, labs = topHits$SNP, cex = textxy.cex), ...)
    }
    else {
      ...
      textxy(topSNPs$pos, -log10(topSNPs$P), offset = 0.625, labs = topSNPs$SNP, cex=textxy.cex)
    }
  }
  par(xpd = FALSE)
}
...
CMplot <- function(
  ...
  highlight.text.arrow.length=0.08,
  highlight.text.arrow.angle=15,
  highlight.text.arrow.lwd=2,
  highlight.text.arrow.col="grey",
  chr.labels=NULL,
  chr.border=FALSE,
  chr.labels.angle=0,
  chr.den.col="black",
  ...
  )
  {
	...
	xadj[x<0] = 1.5
      ...
      for(i in indd){
        ...
        arrows(x1[i-n], y1[i-n], nx, ny, length=arrow.length, angle=arrow.angle, code=2, col=arrow.col, lwd=arrow.lwd)
  ...
  if("c" %in% plot.type){
    ...
    for(i in 1:R){
      ...
      if(outward==TRUE){
        ...
        if(!is.null(highlight)){
          if(highlight.cex>0) points(X[highlight_index[[i]]],X[highlight_index[[i]]],pch=19,cex=cex[1],col="white")
        ...
        if(!is.null(highlight.text)){
          #points(x=X[highlight_index[[i]]],Y[highlight_index[[i]]],pch=pch,cex=cex[2]*R,col="white")
          if(!is.na(highlight_index[[i]][1])){
            if(is.null(highlight.col)){
              highlight_text(X[highlight_index[[i]]],Y[highlight_index[[i]]],xlim=c(min_no_na(X)-band,max_no_na(X)),ylim=c(-Inf,Inf),xadj=highlight.text.xadj[[i]],yadj=highlight.text.yadj[[i]],words=highlight.text[[i]],point.cex=highlight.cex*R,text.cex=highlight.text.cex*R/2, pch=highlight.pch,type=highlight.type,point.col=rep(rep(colx,N[i]),add[[i]])[highlight_index[[i]]],text.col=highlight.text.col,text.font=highlight.text.font,arrow.cex=highlight.text.arrow.cex,arrow.col=highlight.text.arrow.col)
            }else{
              highlight_text(X[highlight_index[[i]]],Y[highlight_index[[i]]],xlim=c(min_no_na(X)-band,max_no_na(X)),ylim=c(-Inf,Inf),xadj=highlight.text.xadj[[i]],yadj=highlight.text.yadj[[i]],words=highlight.text[[i]],point.cex=highlight.cex*R,text.cex=highlight.text.cex*R/2, pch=highlight.pch,type=highlight.type,point.col=highlight.col,text.col=highlight.text.col,text.font=highlight.text.font,arrow.lwd=highlight.text.arrow.lwd,arrow.col=highlight.text.arrow.col,arrow.angle=highlight.text.arrow.angle,arrow.length=highlight.text.arrow.length)
            }
          }
        }
